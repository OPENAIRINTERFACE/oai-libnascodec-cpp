...
{
       int size = 0;
       size += pdu_session_identity.code(data, InformationElement::Format::V);
       size += procedure_transaction_identity.code(data, InformationElement::Format::V);
       size += message_type.code(data, InformationElement::Format::V);
       return size;
}
...
int Pdu5gsSm::decode(const std::vector<uint8_t> &data)
{
       ...
       size = pdu_session_identity.decode_V(data[offset]);
       offset += size;

       tmp.erase(tmp.begin(), tmp.begin() + size);
       size = procedure_transaction_identity.decode_V(data[offset + 1]);
       offset += size;
